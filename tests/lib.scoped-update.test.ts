...
describe("scoped update specs and paths", () => {
  it("accepts only bare, latest, and exact SemVer package specs", () => {
    for (const spec of [
      "@sarctiann/opencode-quota",
      "@sarctiann/opencode-quota@latest",
      "@sarctiann/opencode-quota@3.11.1",
      "@sarctiann/opencode-quota@3.11.2-beta.1+build.2",
    ])
      expect(isCanonicalQuotaUpdateSpec(spec)).toBe(true);
    for (const spec of [
      "@sarctiann/opencode-quota@next",
      "@sarctiann/opencode-quota@^3.11.1",
      "@sarctiann/opencode-quota@~3.11.1",
      "npm:@sarctiann/opencode-quota@3.11.1",
      "file:../opencode-quota",
      "workspace:*",
...
describe("scoped update config planning", () => {
  it("prefers JSONC, preserves comments/format/options, and leaves custom specs untouched", async () => {
    ...
    const original = `{\n  // keep this comment\n  "plugin": [\n    "@sarctiann/opencode-quota@3.11.1",\n    ["@sarctiann/opencode-quota", { "setting": true }],\n    "@sarctiann/opencode-quota@next",\n    "other-plugin",\n  ],\n  "unrelated": { "keep": true },\n}\n`;
    write(jsonc, original);
    write(ignoredJson, `{"plugin":["@sarctiann/opencode-quota@1.0.0"]}`);
    const plan = await planScopedUpdate({
    ...
    expect(updated).toContain('"@sarctiann/opencode-quota@next"');
  ...
  it("honors OPENCODE_CONFIG_DIR and deduplicates project/global real paths", async () => {
    ...
    write(config, `{"plugin":["@sarctiann/opencode-quota"]}`);
    const plan = await planScopedUpdate({
      cwd: f.project,
  ...
  it("aborts planning before writes when any selected config is unparseable", async () => {
    ...
    write(valid, `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`);
  ...
  it("is idempotent after applying its targeted edits", async () => {
    ...
    write(config, `{"plugin":["@sarctiann/opencode-quota@3.11.1","other"]}`);
...
describe("scoped update application safety", () => {
  it("rejects a race in an unchanged @latest config before cache deletion", async () => {
    const f = fixture();
    const config = join(f.project, "opencode.json");
    write(config, `{"plugin":["@sarctiann/opencode-quota@latest"]}`);
    const cache = join(f.cache, "packages", "@sarctiann", "opencode-quota@latest");
    const manifest = join(cache, "node_modules", "@sarctiann", "opencode-quota", "package.json");
    write(manifest, `{"name":"@sarctiann/opencode-quota"}`);
    const plan = await planScopedUpdate({
      cwd: f.project,
      env: f.env,
      homeDir: join(f.root, "home"),
    ...
    expect(readFileSync(manifest, "utf8")).toContain("@sarctiann/opencode-quota");
  });

  it("revalidates @latest authority immediately before deleting cache", async () => {
    const f = fixture();
    const config = join(f.project, "opencode.json");
    write(config, `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`);
    const cache = join(f.cache, "packages", "@sarctiann", "opencode-quota@latest");
    const manifest = join(cache, "node_modules", "@sarctiann", "opencode-quota", "package.json");
    write(manifest, `{"name":"@sarctiann/opencode-quota"}`);
    const plan = await planScopedUpdate({
  ...
  it.each([
    ...
    write(first, `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`);
    write(second, `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`);
    const plan = await planScopedUpdate({
      cwd: f.project,
      env: f.env,
      homeDir: join(f.root, "home"),
  ...
  it("detects raw-byte races before writing", async () => {
    ...
    write(config, `{"plugin":["@sarctiann/opencode-quota@3.11.1"],"raced":true}`);
    await expect(applyScopedUpdatePlan(plan)).rejects.toThrow("changed since preview");
  ...
  it("removes only manifest-verified derived cache directories", async () => {
    ...
    write(
      ...
      `{"plugin":["@sarctiann/opencode-quota@3.11.1","other-plugin"]}`,
    );
    const quotaCache = join(f.cache, "packages", "@sarctiann", "opencode-quota@3.11.1");
    const latestCache = join(f.cache, "packages", "@sarctiann", "opencode-quota@latest");
    const otherCache = join(f.cache, "packages", "other-plugin");
    for (const path of [quotaCache, latestCache])
      write(
        join(path, "node_modules", "@sarctiann", "opencode-quota", "package.json"),
        `{"name":"@sarctiann/opencode-quota"}`,
      );
    write(
  ...
  it("skips symlinks and wrong manifests without broadening deletion", async () => {
    const f = fixture();
    write(join(f.project, "opencode.json"), `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`);
    const outside = join(f.root, "outside");
    mkdirSync(outside);
    const exact = join(f.cache, "packages", "@sarctiann", "opencode-quota@3.11.1");
    mkdirSync(dirname(exact), { recursive: true });
    symlinkSync(outside, exact);
    const latest = join(f.cache, "packages", "@sarctiann", "opencode-quota@latest");
    write(
      join(latest, "node_modules", "@sarctiann", "opencode-quota", "package.json"),
  ...
  it("reports dry-run and cancellation as no-write outcomes", async () => {
    const f = fixture();
    const config = join(f.project, "opencode.json");
    const original = `{"plugin":["@sarctiann/opencode-quota@3.11.1"]}`;
